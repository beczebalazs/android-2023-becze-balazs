...
class RecipesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recipeAdapter = RecipeAdapter(emptyList(), onItemClick =
        ...
        viewModel.recipeList.observe(viewLifecycleOwner){ recipes ->
            for(recipe in recipes){
                ...
                if (nutrition != null) {
                    Log.d(TAG, "Nutrition - Fiber: ${nutrition.fiber}, Protein: ${nutrition.protein}, " +
                ...
                if (components.isNotEmpty()) {
                    ...
                    for (component in components) {
                        Log.d(TAG, "- Raw Text: ${component.rawText}, Extra Comment: ${component.extraComment}")
                    }
                } else {
                    ...
                    for (section in sections) {
                        ...
                        if (componentsInSection.isNotEmpty()) {
                            ...
                            for (componentInSection in componentsInSection) {
                                Log.d(TAG, "-- Raw Text: ${componentInSection.rawText}, Extra Comment: ${componentInSection.extraComment}")

                                val ingredient = componentInSection.ingredient
                                Log.d(TAG, "--- Ingredient: Name: ${ingredient.name}, ID: ${ingredient.id}, Display Singular: ${ingredient.displaySingular}, " +
                                ...
                                if (measurements.isNotEmpty()) {
                                    ...
                                    for (measurement in measurements) {
                                        Log.d(TAG, "---- Quantity: ${measurement.quantity}, Unit: Name: ${measurement.unit.name}, " +
                ...
                if (tags.isNotEmpty()) {
                    Log.d(TAG, "Tags:")
                    for (tag in tags) {
                        Log.d(TAG, "- Root Tag Type: ${tag.rootTagType}, Name: ${tag.name}, ID: ${tag.id}, Display Name: ${tag.displayName}, Type: ${tag.type}")
                    }
                } else {
    ...
    private fun navigateToRecipeDetail(recipe: RecipeModel) {
        findNavController()
            .navigate(
                ...
                bundleOf("recipeId" to recipe.name)
            )
    }
}